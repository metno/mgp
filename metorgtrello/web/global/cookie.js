// The code in this file is one of the examples of
// David Flanagan: JavaScript - The Definitive Guide, 5th ed., O'Reilly 2006.
// It was downloaded from http://oreilly.com/catalog/jscript5
// and is used in accordance with the the permissions stated
// in the book (page xix of the Preface).
...
function Cookie(name) {
    ...
    this.dynProps = []; // Dynamic properties (i.e. an associative array)
    ...
    // --- BEGIN Option 1: Static properties -------------------------------------------
//         this[a[i][0]] = decodeURIComponent(a[i][1]);
    // --- END Option 1 ----------------------------------------------------------------
    // --- BEGIN Option 2: Dynamic properties ------------------------------------------
        this.dynProps[a[i][0]] = decodeURIComponent(a[i][1]);
    // --- END Option 2 ----------------------------------------------------------------
    }
}

Cookie.prototype.setDynamicProperty = function(name, value)
{
    this.dynProps[name] = value;
}

Cookie.prototype.getDynamicProperties = function()
{
...
//     }
    ...
    // --- BEGIN Option 2: Dynamic properties ------------------------------------------
    for (name in this.dynProps) {
        if (cookieval != "") cookieval += '&';
        cookieval += name + ':' + encodeURIComponent(this.dynProps[name]);
    }
    // --- END Option 2 ----------------------------------------------------------------
    ...
    var dtl = (daysToLive || daysToLive == 0) ? daysToLive : 365;
    cookie += "; max-age=" + (dtl * 24 * 60 * 60);
